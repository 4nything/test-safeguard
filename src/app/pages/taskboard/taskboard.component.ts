...
export class TaskboardComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        this.dragulaService.createGroup(this.list, {
            ...
            accepts: (_el, target, source, _sibling) => {
                if (source?.id === "list1" && target?.id === "list3") return false;
                if (source?.id === "list3" && target?.id === "list2") return false;
                return true;
            }
        });
    ...
    setData() {
        ['ListItem 1', 'ListItem 2', 'ListItem 3'].map(t => this.list1.push({ title: t, theme: 'theme-1', origin: 1 }));
        ['ListItem 4', 'ListItem 5', 'ListItem 6'].map(t => this.list2.push({ title: t, theme: 'theme-2', origin: 2 }));
        ['ListItem 7', 'ListItem 8', 'ListItem 9'].map(t => this.list3.push({ title: t, theme: 'theme-3', origin: 3 }));
    }
}
